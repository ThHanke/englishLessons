...
describe('agentSession', () => {
  ...
  it('calls query() without resume for a new class+date and persists the returned session id', async () => {
    ...
    const outcome = await runAgentTurn({ classId: 'grade5a', date: '2026-09-01', prompt: 'hi', cwd: REPO_CWD });

    expect(queryMock).toHaveBeenCalledTimes(1);
    const options = queryMock.mock.calls[0]![0].options as Options;
    expect(options.resume).toBeUndefined();
    expect(outcome.sessionId).toBe('brand-new-session');
    await expect(getSessionId({ classId: 'grade5a', date: '2026-09-01' })).resolves.toBe('brand-new-session');
  });
  ...
  it('calls query() with resume: <stored id> for an existing class+date', async () => {
    ...
    await setSessionId({ classId: 'grade5a', date: '2026-09-01', sessionId: 'prior-session' });
    queryMock.mockReturnValue(fakeStream([resultMessage('prior-session')]));

    await runAgentTurn({ classId: 'grade5a', date: '2026-09-01', prompt: 'continue', cwd: REPO_CWD });
  ...
  it('keeps two different dates for the same class as distinct index entries', async () => {
    ...
    await runAgentTurn({ classId: 'grade5a', date: '2026-09-01', prompt: 'a', cwd: REPO_CWD });
    await runAgentTurn({ classId: 'grade5a', date: '2026-09-03', prompt: 'b', cwd: REPO_CWD });

    await expect(getSessionId({ classId: 'grade5a', date: '2026-09-01' })).resolves.toBe('session-mon');
    await expect(getSessionId({ classId: 'grade5a', date: '2026-09-03' })).resolves.toBe('session-wed');
  });
  ...
  it('always includes Edit/Write/MultiEdit/NotebookEdit/Bash in disallowedTools', async () => {
    ...
    await runAgentTurn({ classId: 'grade5a', date: '2026-09-01', prompt: 'hi', cwd: REPO_CWD });
  ...
  it('does not terminate the conversation when a mid-conversation tool-result is a denied write', async () => {
    ...
    const deniedToolResult = {
      ...
      message: {
        role: 'user',
        content: [{ type: 'tool_result', is_error: true, content: 'Permission denied: Write is disallowed' }],
      },
      session_id: 's1',
    };
    queryMock.mockReturnValue(
      fakeStream([
        deniedToolResult,
        resultMessage('s1', {
          permission_denials: [{ tool_name: 'Write', tool_use_id: 'x', tool_input: {} }] as never,
        }),
      ]),
    );

    const outcome = await runAgentTurn({ classId: 'grade5a', date: '2026-09-01', prompt: 'edit the file', cwd: REPO_CWD });
  ...
  it('starts a fresh session with a notice when resuming a stored id fails (missing/corrupted transcript)', async () => {
    ...
    await setSessionId({ classId: 'grade5a', date: '2026-09-01', sessionId: 'broken-session' });
    ...
    await expect(getSessionId({ classId: 'grade5a', date: '2026-09-01' })).resolves.toBe('fresh-session');
  });
  ...
  it('does not corrupt the index when two overlapping requests hit the same class+date', async () => {
    ...
    await Promise.all([
      runAgentTurn({ classId: 'grade5a', date: '2026-09-01', prompt: 'a', cwd: REPO_CWD }),
      runAgentTurn({ classId: 'grade5a', date: '2026-09-01', prompt: 'b', cwd: REPO_CWD }),
    ]);
