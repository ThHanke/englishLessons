 ...
 */
export async function handleLessonPreviewRequest(req: IncomingMessage, res: ServerResponse, config: { repoRoot?: string }): Promise<void> {
  const url = new URL(req.url ?? '/', 'http://127.0.0.1');
  const className = url.searchParams.get('class');
  const date = url.searchParams.get('date');

  if (!className || !date) {
    sendJson(res, 400, { error: 'missing_query_params', required: ['class', 'date'] });
    return;
  }

  try {
    const context: DateContext = dateContext({ className, date, repoRoot: config.repoRoot });
