...
describe('handleLessonPreviewRequest (HTTP)', () => {
  ...
  beforeEach(async () => {
    server = createServer((req, res) => {
      void handleLessonPreviewRequest(req, res, { repoRoot: FIXTURE_REPO_ROOT });
    });
  ...
  it('returns 400 when class or date is missing', async () => {
    const res = await fetch(`${baseUrl}/api/lesson-preview?class=fixture-class`);
    expect(res.status).toBe(400);
  });

  it('returns the teaching-day context for a valid class + date', async () => {
    const res = await fetch(`${baseUrl}/api/lesson-preview?class=fixture-class&date=2026-08-03`);
  ...
  it('returns the non-teaching-day context for a weekend', async () => {
    const res = await fetch(`${baseUrl}/api/lesson-preview?class=fixture-class&date=2026-08-08`);
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  it('returns 500 for an unknown class', async () => {
    const res = await fetch(`${baseUrl}/api/lesson-preview?class=nonexistent-class&date=2026-08-03`);
