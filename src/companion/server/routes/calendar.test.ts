...
describe('buildCalendarRange', () => {
  ...
  it('flags a non-teaching day distinctly, with module/phase/gap fields null', () => {
    ...
    expect(days[0]!.reason).toMatch(/holiday, weekend, or outside the school year/);
  });
...
describe('handleCalendarRequest (HTTP)', () => {
  ...
  it('returns whichModule/gapReport-derived data shaped for the calendar UI', async () => {
    const res = await fetch(`${baseUrl}/api/calendar?class=fixture-class&from=2026-08-03&to=2026-08-05`);
    expect(res.status).toBe(200);
    const body = (await res.json()) as { className: string; days: Array<{ date: string; moduleId: string | null }> };
  ...
  it('responds 400 when a required query param is missing', async () => {
    const res = await fetch(`${baseUrl}/api/calendar?class=fixture-class&from=2026-08-03`);
