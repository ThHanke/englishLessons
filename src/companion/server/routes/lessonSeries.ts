...
function loadFirstCalendar(
  repoRoot: string,
): { calendar: CalendarFile; path: string } | null {
...
export async function handleSeriesPreviewRequest(
  ...
  if (!className || !day || !start || !end || !halfYearRaw) {
    sendJson(res, 400, {
      ...
      required: ["class", "day", "start", "end", "halfYear"],
    });
    return;
  }

  const halfYear = Number(halfYearRaw);
  const validation = validateSeriesInput({
    className,
    day,
    start,
    end,
...
export async function handleCreateSeriesRequest(
  ...
  if (
    ...
    !start ||
    !end ||
    halfYearRaw === undefined ||
    !from ||
    !to
  ) {
    sendJson(res, 400, {
      error: "missing_fields",
      required: ["className", "day", "start", "end", "halfYear", "from", "to"],
    });
    return;
  }
  ...
  try {
    const calResult =
      findCalendarForClass(className, config.repoRoot) ??
      loadFirstCalendar(config.repoRoot);
    if (!calResult) {
      sendJson(res, 500, { error: "no calendar file found" });
      return;
    }
