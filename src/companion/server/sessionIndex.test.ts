...
import { getSessionId, getSessionIndexPath, setSessionId } from './sessionIndex.ts';
...
describe('sessionIndex', () => {
  ...
  it('returns undefined for a class+date with no stored session', async () => {
    tmpDirs.push(isolateConfigDir());
    await expect(getSessionId({ classId: 'grade5a', date: '2026-09-01' })).resolves.toBeUndefined();
  });

  it('persists and retrieves a session id for a class+date', async () => {
    tmpDirs.push(isolateConfigDir());
    await setSessionId({ classId: 'grade5a', date: '2026-09-01', sessionId: 'session-1' });
    await expect(getSessionId({ classId: 'grade5a', date: '2026-09-01' })).resolves.toBe('session-1');
  });

  it('keeps two different dates for the same class as distinct entries', async () => {
    tmpDirs.push(isolateConfigDir());
    await setSessionId({ classId: 'grade5a', date: '2026-09-01', sessionId: 'session-mon' });
    await setSessionId({ classId: 'grade5a', date: '2026-09-03', sessionId: 'session-wed' });
  ...
  it('does not collide across different classIds on the same date', async () => {
    tmpDirs.push(isolateConfigDir());
    await setSessionId({ classId: 'grade5a', date: '2026-09-01', sessionId: 'session-5a' });
    await setSessionId({ classId: 'grade6b', date: '2026-09-01', sessionId: 'session-6b' });

    await expect(getSessionId({ classId: 'grade5a', date: '2026-09-01' })).resolves.toBe('session-5a');
    await expect(getSessionId({ classId: 'grade6b', date: '2026-09-01' })).resolves.toBe('session-6b');
  });
  ...
  it('serializes two overlapping writes for the same class+date without corrupting the index', async () => {
    ...
    await Promise.all([
      setSessionId({ classId: 'grade5a', date: '2026-09-01', sessionId: 'session-a' }),
      setSessionId({ classId: 'grade5a', date: '2026-09-01', sessionId: 'session-b' }),
    ]);
    ...
    const stored = await getSessionId({ classId: 'grade5a', date: '2026-09-01' });
