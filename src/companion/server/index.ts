 ...
 */
export async function createCompanionServer(params?: { port?: number; repoRoot?: string }): Promise<CompanionServerHandle> {
  ...
  const httpServer = createHttpServer((req, res) => {
    ...
    if (req.method === 'GET' && url.pathname === '/api/session-token') {
      ...
      res.end(JSON.stringify({ token: sessionToken, issuedAt: new Date().toISOString() }));
      return;
    }
    ...
    if (req.method === 'POST' && url.pathname === '/api/chat') {
      void handleChatRequest(req, res, { expectedOrigin: origin, sessionToken, cwd: repoRoot });
      return;
    }
