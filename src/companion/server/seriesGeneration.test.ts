...
describe('validateSeriesInput', () => {
  it('accepts valid input', () => {
    const result = validateSeriesInput({ className: 'grade-7', day: 'Mon', start: '08:00', end: '09:30', halfYear: 1 });
    expect(result).toEqual({ valid: true });
  });

  it('rejects bad className (path traversal)', () => {
    const result = validateSeriesInput({ className: '../evil', day: 'Mon', start: '08:00', end: '09:30', halfYear: 1 });
    expect(result.valid).toBe(false);
  });

  it('rejects halfYear = 3', () => {
    const result = validateSeriesInput({ className: 'grade-7', day: 'Mon', start: '08:00', end: '09:30', halfYear: 3 });
    expect(result.valid).toBe(false);
  });

  it('rejects day = "Sunday"', () => {
    const result = validateSeriesInput({ className: 'grade-7', day: 'Sunday', start: '08:00', end: '09:30', halfYear: 1 });
    expect(result.valid).toBe(false);
  });

  it('rejects bad time format', () => {
    const result = validateSeriesInput({ className: 'grade-7', day: 'Mon', start: '8:00', end: '09:30', halfYear: 1 });
    expect(result.valid).toBe(false);
  });
});
...
describe('validateSlotId', () => {
  it('accepts a valid UUID', () => {
    expect(validateSlotId('a1b2c3d4-e5f6-7890-abcd-ef1234567890')).toEqual({ valid: true });
  });
...
describe('generateSeriesDates', () => {
  ...
  it('skips holidays', () => {
    const calendar = makeCalendar({
      ...
      holidays: [{ name: 'Test Holiday', from: '2026-08-03', to: '2026-08-03' }],
    });
    const dates = generateSeriesDates({ calendar, day: 'Mon', halfYear: 1 });
...
describe('seriesPreview', () => {
  it('detects conflict when another class has same day+time+halfYear', () => {
    const calendar = makeCalendar({
      ...
      class_schedule: {
        'other-class': {
          lesson_slots: [{ id: 'existing-id', day: 'Mon', start: '08:00', end: '09:30', half_year: 1 }],
        },
      },
    });
...
describe('persistSeries', () => {
  it('adds slot to existing class entry', async () => {
    ...
    try {
      ...
      const calendar = makeCalendar({
        class_schedule: { 'my-class': { lesson_days: ['Mon'] } },
      });
      ...
      await persistSeries({
        ...
        className: 'my-class',
        slot: { id: 'slot-1', day: 'Mon', start: '08:00', end: '09:30', half_year: 1 },
      });
  ...
  it('creates new class_schedule entry when absent', async () => {
    ...
    try {
      ...
      await persistSeries({
        calendarPath: calPath,
        className: 'new-class',
        slot: { id: 'slot-2', day: 'Tue', start: '10:00', end: '11:30', half_year: 2 },
      });
    ...
    try {
      ...
      const calendar = makeCalendar({
        class_schedule: {
          'my-class': {
            lesson_slots: [
              { id: 'keep-me', day: 'Mon', start: '08:00', end: '09:30', half_year: 1 },
              { id: 'delete-me', day: 'Wed', start: '10:00', end: '11:30', half_year: 1 },
            ],
          },
        },
      });
      ...
      await deleteSeries({ calendarPath: calPath, className: 'my-class', slotId: 'delete-me' });
    ...
    try {
      ...
      const calendar = makeCalendar({
        class_schedule: {
          'my-class': {
            lesson_slots: [
              { id: 'existing', day: 'Mon', start: '08:00', end: '09:30', half_year: 1 },
            ],
          },
        },
      });
      ...
      await deleteSeries({ calendarPath: calPath, className: 'my-class', slotId: 'nonexistent' });
...
describe('write mutex serialization', () => {
  it('rapid writes serialize correctly', async () => {
    ...
    try {
      const calPath = join(tmpDir, 'calendar.yaml');
      const calendar = makeCalendar({ class_schedule: { 'my-class': { lesson_days: [] } } });
      writeYaml(calPath, calendar);
      ...
      const promises = Array.from({ length: 5 }, (_, i) =>
        persistSeries({
          calendarPath: calPath,
          className: 'my-class',
          slot: { id: `slot-${i}`, day: 'Mon', start: '08:00', end: '09:30', half_year: 1 },
        }),
      );
