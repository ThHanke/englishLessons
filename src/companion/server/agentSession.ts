...
- \`stages\`: \`[{ name, durationMinutes, purpose, procedure, materialRefs }]\` — the timed structure (warm-up/review → input → guided practice → production → wrap-up is the usual shape; adapt to what the lesson actually needs). A degraded pace (slower module progress) should shorten the new-input stage and expand review, not silently keep the same split.
  ...
  - \`procedure\`: ordered steps, each \`{ kind, text, durationMinutes? }\` — not one paragraph. This is what makes the stage readable at a glance instead of a wall of text. \`kind\` is \`"teacher_intro"\` (you're explaining/modelling), \`"pupil_work"\` (pupils work alone/in pairs — give it a \`durationMinutes\` and the lesson page renders a Start-timer button pupils can click and count down), or \`"correction"\` (reviewing answers together). Only mark a step \`pupil_work\` if it's genuinely autonomous — not you demonstrating. Step durations for a stage should roughly sum to that stage's own \`durationMinutes\`.
