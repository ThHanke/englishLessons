  ...
  it("GET /api/calendar returns module/phase/gap data for a date range", async () => {
    ...
    try {
      const res = await fetch(
        `${handle.url}/api/calendar?class=fixture-class&from=2026-08-03&to=2026-08-03`,
      );
      expect(res.status).toBe(200);
      const body = (await res.json()) as {
        days: Array<{ moduleId: string | null }>;
      };
  ...
  it("streams incremental assistant output over multiple chunks for a valid request, not one buffered response", async () => {
    ...
    try {
      ...
      const complete = parsed.find(
        (m) => m.type === "companion_turn_complete",
      ) as
        { type: string; sessionId: string; startedFresh: boolean } | undefined;
