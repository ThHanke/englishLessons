...
export function generateSeriesDates(params: {
  ...
  while (cursor <= range.to) {
    ...
    if (
      ...
      !params.calendar.events
        .filter((e) => matchesEvent(cursor, e))
        .some((e) => e.capacity === 0)
    ) {
      dates.push(cursor);
    }
...
export function seriesPreview(params: {
  ...
  for (const [classId, entry] of Object.entries(
    params.calendar.class_schedule,
  )) {
...
export async function persistSeries(params: {
  ...
  await withYamlLock(() => {
    ...
    if (!calendar.class_schedule[params.className]) {
      calendar.class_schedule[params.className] = {};
    }
...
export async function deleteSeries(params: {
  ...
  await withYamlLock(() => {
    ...
    entry.lesson_slots = entry.lesson_slots.filter(
