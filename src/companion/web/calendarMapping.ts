...
export function groupColorClass(classId: string, order: Map<string, number>): string {
...
const GAP_RANK: Record<GapKind, number> = { uncovered: 0, 'under-depth': 1, 'at-risk': 2 };
...
export function worstGapSeverity(task: ModuleTask): GapKind | null {
  ...
  for (const gap of task.gaps) {
    if (worst === null || GAP_RANK[gap.kind] > GAP_RANK[worst]) worst = gap.kind;
  }
  return worst;
}
...
export function taskEventClass(task: ModuleTask, order: Map<string, number>): string {
