...
export function mapSchoolHolidays(response: OpenHolidaysPeriod[]): Holiday[] {
  if (!Array.isArray(response) || response.length === 0) {
    throw new Error('OpenHolidaysAPI /SchoolHolidays returned an empty or malformed response');
  }
  return response.map((period) => {
    if (!period.startDate || !period.endDate) {
      throw new Error(`Malformed holiday period: missing startDate/endDate (id-less entry: ${JSON.stringify(period)})`);
    }
    const name = period.name.find((n) => n.language === 'EN')?.text ?? period.name[0]?.text;
    if (!name) {
      throw new Error(`Malformed holiday period: no name entries (${JSON.stringify(period)})`);
    }
...
export function mapPublicHolidays(response: OpenHolidaysPeriod[]): Holiday[] {
  if (!Array.isArray(response)) {
    throw new Error('OpenHolidaysAPI /PublicHolidays returned a malformed response');
  }
  return response.map((period) => {
    if (!period.startDate || !period.endDate) {
      throw new Error(`Malformed public holiday entry: missing startDate/endDate (${JSON.stringify(period)})`);
    }
    const name = period.name.find((n) => n.language === 'EN')?.text ?? period.name[0]?.text;
    if (!name) {
      throw new Error(`Malformed public holiday entry: no name entries (${JSON.stringify(period)})`);
    }
    return { name, from: period.startDate, to: period.endDate };
  });
}
 ...
 */
export function mergeHolidays(schoolHolidays: Holiday[], publicHolidays: Holiday[]): Holiday[] {
  const isContained = (h: Holiday) => schoolHolidays.some((s) => h.from >= s.from && h.to <= s.to);
...
export function deriveSchoolYearBounds(holidays: Holiday[]): {
  ...
  if (summer.length < 2) {
    throw new Error(`Expected two Summer Holiday entries to bound the school year, found ${summer.length}`);
  }
  return {
...
export function buildCalendarFile(params: {
  ...
  const { first_school_day, last_school_day } = deriveSchoolYearBounds(schoolHolidays);
  ...
  return {
    ...
    class_schedule: {
      [params.className]: { lesson_days: params.lessonDays },
    },
  };
}

async function fetchHolidayEndpoint(endpoint: 'SchoolHolidays' | 'PublicHolidays', subdivisionCode: string, validFrom: string, validTo: string): Promise<OpenHolidaysPeriod[]> {
  ...
  if (!res.ok) {
    throw new Error(`OpenHolidaysAPI request failed: ${res.status} ${res.statusText}`);
  }
...
async function main(): Promise<void> {
  ...
  const outPath = new URL('../../calendar/sachsen-anhalt-2026-2027.yaml', import.meta.url).pathname;
