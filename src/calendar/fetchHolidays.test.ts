...
const fixturePath = new URL('./fixtures/school-holidays-response.json', import.meta.url).pathname;
const fixture = JSON.parse(readFileSync(fixturePath, 'utf-8')) as OpenHolidaysPeriod[];
const publicFixturePath = new URL('./fixtures/public-holidays-response.json', import.meta.url).pathname;
const publicFixture = JSON.parse(readFileSync(publicFixturePath, 'utf-8')) as OpenHolidaysPeriod[];
...
describe('mapSchoolHolidays', () => {
  it('maps the real fixture response to the correct number of holidays[] entries with name/from/to', () => {
    ...
    expect(holidays[0]).toEqual({ name: 'Summer Holidays', from: '2026-07-04', to: '2026-08-14' });
    expect(holidays[2]).toEqual({ name: 'Christmas Holidays', from: '2026-12-21', to: '2027-01-02' });
  });
  ...
  it('throws a clear error on a malformed period missing dates', () => {
    expect(() => mapSchoolHolidays([{ startDate: '', endDate: '', type: 'School', name: [{ language: 'EN', text: 'X' }] }])).toThrow();
  });
...
describe('deriveSchoolYearBounds', () => {
  it('derives first/last school day from the two Summer-holiday boundary entries', () => {
    ...
    expect(bounds).toEqual({ first_school_day: '2026-08-15', last_school_day: '2027-07-09' });
  });

  it('throws when fewer than two Summer Holiday entries are present', () => {
    expect(() => deriveSchoolYearBounds([{ name: 'Autumn Holidays', from: '2026-10-19', to: '2026-10-30' }])).toThrow();
  });
});
...
describe('buildCalendarFile', () => {
  it('builds a full CalendarFile from the real fixtures, merging public into school holidays', () => {
    ...
    expect(calendar.class_schedule['grade-7-realschule-2026']).toEqual({ lesson_days: ['Mon', 'Wed', 'Fri'] });
  });
});
