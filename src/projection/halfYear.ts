...
export function deriveHalfYearBoundary(calendar: CalendarFile): string {
  ...
  const winterHoliday = calendar.holidays.find(
    (h) => h.name === "Winter Holidays",
  );
  if (!winterHoliday) {
    throw new Error(
      'Calendar has no half_year_boundary field and no "Winter Holidays" holiday entry — cannot derive the Halbjahr boundary',
    );
  }
