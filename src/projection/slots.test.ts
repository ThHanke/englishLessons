...
const shortGapCalendar = loadYaml<CalendarFile>(
  new URL("./fixtures/calendar-short-gap.yaml", import.meta.url).pathname,
);
const noHolidaysCalendar = loadYaml<CalendarFile>(
  new URL("./fixtures/calendar-no-holidays.yaml", import.meta.url).pathname,
);
...
describe("enumerateSlots", () => {
  it("yields the expected slot count with no slots inside the holiday range", () => {
    ...
    expect(
      slots.some((s) => s.date >= "2026-09-14" && s.date <= "2026-09-18"),
    ).toBe(false);
...
describe("weightSlots", () => {
  ...
  it("applies post_holiday_factor to the first slots after a holiday", () => {
    const slots = enumerateSlots(smallCalendar, "test-class");
    const weighted = weightSlots(slots, smallCalendar);
    expect(weighted.find((s) => s.date === "2026-09-21")!.weight).toBeCloseTo(
      1 * 0.8,
    );
    expect(weighted.find((s) => s.date === "2026-09-23")!.weight).toBeCloseTo(
      1 * 0.8,
    );
    expect(weighted.find((s) => s.date === "2026-09-25")!.weight).toBeCloseTo(
      1,
    );
  });
  ...
  it("degrades nothing when pre_holiday_days is 0 (slice(-0) boundary, not the whole array)", () => {
    const zeroPreCalendar: CalendarFile = {
      ...smallCalendar,
      pace_factors: { ...smallCalendar.pace_factors, pre_holiday_days: 0 },
    };
...
describe("enumerateSlots with lesson_slots", () => {
  ...
  it("Mon H1 + Thu H2 yields Mon in H1, Thu in H2", () => {
    ...
    const thuSlots = slots.filter(
      (s) => new Date(s.date + "T00:00:00Z").getUTCDay() === 4,
    );
  ...
  it("lesson_slots for Mon/Wed/Fri produces slots only on those days", () => {
    const cal = makeSlotCalendar({
  ...
  it("holidays are still skipped when using lesson_slots path", () => {
    const cal = makeSlotCalendar({
      half_year_boundary: "2026-09-15",
      holidays: [
        { name: "Test Holiday", from: "2026-09-07", to: "2026-09-07" },
      ],
      class_schedule: {
        cls: {
          lesson_slots: [
  ...
  it("empty class_schedule entry yields empty", () => {
    const cal = makeSlotCalendar({
      class_schedule: { cls: {} },
    });
    const slots = enumerateSlots(cal, "cls");
...
describe("weightSlots with double-period slots", () => {
  it("double-period Monday before holiday: both slots degraded, only one day consumed from N-day window", () => {
    ...
    const cal = makeSlotCalendar({
      ...
      holidays: [
        { name: "Test Holiday", from: "2026-09-14", to: "2026-09-18" },
      ],
      class_schedule: {
        cls: {
          lesson_slots: [
